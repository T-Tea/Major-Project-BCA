...
function delete_hosteller() {
    ...
    try {
        ...
        if ($stmt->execute()) {
            echo json_encode(['status' => 'success', 'message' => 'Hosteller deleted successfully']);
        } else {
            throw new Exception("Error: " . $stmt->error);
        }
...
// Function to add new warden
function add_warden() {
    global $conn;
    
    try {
        // Get form data
        $name = sanitize_input($_POST['warden-name']);
        $building = sanitize_input($_POST['warden-building']);
        $contact = sanitize_input($_POST['warden-contact']);
        $password = sanitize_input($_POST['warden-password']);
        $address = sanitize_input($_POST['warden-address']);
        
        // Hash password
        $hashed_password = password_hash($password, PASSWORD_DEFAULT);
        
        // Prepare and execute SQL statement
        $stmt = $conn->prepare("INSERT INTO warden (name, password, address, phone_number, building) VALUES (?, ?, ?, ?, ?)");
        $stmt->bind_param("sssss", $name, $hashed_password, $address, $contact, $building);
        
        if ($stmt->execute()) {
            echo json_encode(['status' => 'success', 'message' => 'Warden added successfully']);
        } else {
            throw new Exception("Error: " . $stmt->error);
        }
...
// Function to update existing warden
function update_warden() {
    global $conn;
    
    try {
        // Get form data
        $id = sanitize_input($_POST['warden-id']);
        $name = sanitize_input($_POST['warden-name']);
        $building = sanitize_input($_POST['warden-building']);
        $contact = sanitize_input($_POST['warden-contact']);
        $password = sanitize_input($_POST['warden-password']);
        ...
        if (!empty($password)) {
            ...
            // Update with new password
            $stmt = $conn->prepare("UPDATE warden SET name=?, password=?, address=?, phone_number=?, building=? WHERE id=?");
            $stmt->bind_param("sssssi", $name, $hashed_password, $address, $contact, $building, $id);
        } else {
            // Update without changing password
            $stmt = $conn->prepare("UPDATE warden SET name=?, address=?, phone_number=?, building=? WHERE id=?");
            $stmt->bind_param("ssssi", $name, $address, $contact, $building, $id);
        }
...
function delete_warden() {
    ...
    try {
        ...
        $stmt = $conn->prepare("DELETE FROM warden WHERE id=?");
        $stmt->bind_param("i", $id);
        
        if ($stmt->execute()) {
            echo json_encode(['status' => 'success', 'message' => 'Warden deleted successfully']);
        } else {
            throw new Exception("Error: " . $stmt->error);
        }
...
<head>
    ...
    <link rel="stylesheet" href="styles/adManage.css">
  ...
  <body>
    <div class="dashboard-container">
      ...
      <div class="right-content">
        ...
        <div class="right-container">
          ...
          <!-- Toggle buttons for Hosteller/Warden management -->
          <div class="toggle-view">
            <button class="toggle-btn active" data-target="hosteller-section">Manage Hostellers</button>
            <button class="toggle-btn" data-target="warden-section">Manage Wardens</button>
          </div>
          ...
          <div id="hosteller-section" class="content-section active">
            <div class="database-section">
              <div class="section-header">
                ...
                <button id="add-hosteller-btn" class="primary-btn"><i class="fas fa-plus"></i> Add New Hosteller</button>
              </div>
          ...
          <!-- Warden Database Management Content -->
          <div id="warden-section" class="content-section">
            <div class="database-section">
              <div class="section-header">
                <h2><i class="fas fa-user-shield"></i> Warden Database</h2>
                <button id="add-warden-btn" class="primary-btn"><i class="fas fa-plus"></i> Add New Warden</button>
              </div>

              <!-- Simple Search for Wardens -->
              <div class="warden-search">
                <input type="text" id="warden-name-search" class="warden-search-input" placeholder="Search wardens by name...">
                <button id="search-warden-btn" class="primary-btn"><i class="fas fa-search"></i> Search</button>
                <button id="clear-warden-search" class="secondary-btn"><i class="fas fa-undo"></i> Clear</button>
              </div>
              
              <!-- Results Summary -->
              <div class="results-summary">
                <p><span id="warden-results-count">0</span> wardens found</p>
              </div>
              
              <!-- Warden Table View -->
              <div class="table-view active-view">
                <table class="hosteller-table warden-table">
                  <thead>
                    <tr>
                      <th>ID</th>
                      <th>Name</th>
                  ...
                  <tbody id="warden-table-body">
                    <!-- Warden data will be loaded here -->
                  </tbody>
                </table>
                
                <div class="pagination" id="warden<div class="pagination" id="warden-pagination">
                  <!-- Pagination will be populated dynamically -->
                </div>
              </div>
            </div>
    ...
    <!-- Modal for Add New Hosteller -->
    <div id="hosteller-modal" class="modal">
      <div class="modal-content">
        <div class="modal-header">
          <h2><i class="fas fa-user-plus"></i> <span id="hosteller-modal-title">Add New Hosteller</span></h2>
          <span class="close-modal">&times;</span>
        </div>
        <div class="modal-body">
          <form id="add-hosteller-form">
            <input type="hidden" id="hosteller-id" name="hosteller-id" value="">
            <div class="form-row">
              <div class="form-group full-width">
                <label for="hosteller-name">Full Name*</label>
                <input type="text" id="hosteller-name" name="hosteller-name" required>
              </div>
            </div>

            <div class="form-row">
              <div class="form-group">
                <label for="hosteller-course">Course*</label>
                <select id="hosteller-course" name="hosteller-course" required>
              ...
              <div class="form-group">
                <label for="hosteller-semester">Semester*</label>
                <select id="hosteller-semester" name="hosteller-semester" required>
            ...
            <div class="form-row">
              <div class="form-group">
                <label for="hosteller-room">Room Number*</label>
                <input type="text" id="hosteller-room" name="hosteller-room" required>
              </div>
              <div class="form-group">
                <label for="hosteller-contact">Contact Number*</label>
                <input type="tel" id="hosteller-contact" name="hosteller-contact" pattern="[0-9]{10}" placeholder="10-digit mobile number" required>
              </div>
            </div>

            <div class="form-row">
              <div class="form-group full-width">
                <label for="hosteller-password">Password*</label>
                <input type="password" id="hosteller-password" name="hosteller-password">
                <small id="password-info">Leave blank to keep current password (for updates only)</small>
              </div>
            </div>

            <div class="form-actions">
              <button type="button" class="secondary-btn" id="cancel-hosteller-btn">Cancel</button>
              <button type="submit" class="primary-btn" id="save-hosteller-btn"><i class="fas fa-save"></i> Save Hosteller</button>
            </div>
          </form>
        </div>
      </div>
    ...
    <!-- Modal for Add New Warden -->
    <div id="warden-modal" class="modal">
      <div class="modal-content">
        <div class="modal-header">
          <h2><i class="fas fa-user-shield"></i> <span id="warden-modal-title">Add New Warden</span></h2>
          <span class="close-modal">&times;</span>
        </div>
        <div class="modal-body">
          <form id="add-warden-form">
            <input type="hidden" id="warden-id" name="warden-id" value="">
            <div class="form-row">
              <div class="form-group full-width">
                <label for="warden-name">Full Name*</label>
                <input type="text" id="warden-name" name="warden-name" required>
              </div>
            </div>

            <div class="form-row">
              <div class="form-group">
                <label for="warden-building">Assigned Building*</label>
                <select id="warden-building" name="warden-building" required>
                  <option value="">Select Building</option>
                  <option value="Boys">Boys Building</option>
                  <option value="Girls">Girls Building</option>
                </select>
              </div>
              <div class="form-group">
                <label for="warden-contact">Contact Number*</label>
                <input type="tel" id="warden-contact" name="warden-contact" pattern="[0-9]{10}" placeholder="10-digit mobile number" required>
              </div>
            </div>
            ...
            <div class="form-row">
              <div class="form-group full-width">
                ...
                <input type="password" id="warden-password" name="warden-password">
                <small id="warden-password-info">Leave blank to keep current password (for updates only)</small>
              </div>
            </div>

            <div class="form-group full-width">
                <label for="warden-address">Address</label>
                <input type="text" id="warden-address" name="warden-address" required>
              </div>

            <div class="form-actions">
              <button type="button" class="secondary-btn" id="cancel-warden-btn">Cancel</button>
              <button type="submit" class="primary-btn" id="save-warden-btn"><i class="fas fa-save"></i> Save Warden</button>
            </div>
          </form>
        </div>
      </div>
    </div>
    <script src="scripts/adManage.js"></script>
