...
function getRoomsWithReports($conn) {
    ...
    $sql = "SELECT DISTINCT h.room 
            ...
            WHERE r.status = 'unread' AND r.recipient = 'admin' 
            ORDER BY h.room ASC";
...
// Function to count total unread reports for admin - FIXED to count ALL unread reports
function getUnreadReportsCount($conn) {
    $sql = "SELECT COUNT(*) as count FROM reports WHERE status = 'unread' AND recipient = 'admin'";
    $result = $conn->query($sql);
...
// Function to get all available rooms with reports for admin
...
function getReportByRoom($conn, $room) {
    if ($room == 'all') {
        ...
        $sql = "SELECT r.*, h.room, h.name as student_name 
                FROM reports r 
                JOIN hostellers h ON r.user_id = h.id 
                WHERE h.room = '$room' AND r.recipient = 'admin' 
                ORDER BY r.created_at DESC LIMIT 1";
    }
...
if (isset($_POST['mark_all'])) {
    $sql = "UPDATE reports SET status = 'read' WHERE status = 'unread' AND recipient = 'admin'";
    $conn->query($sql);
    header("Location: " . $_SERVER['PHP_SELF']);
    exit();
}
